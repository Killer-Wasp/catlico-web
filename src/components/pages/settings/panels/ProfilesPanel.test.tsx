...
describe('ProfilesPanel — built-in roles', () => {
  it('shows a "Built-in" badge only for built-in roles', async () => {
    renderPanel()
    // Both role buttons render...
    await screen.findByRole('button', { name: 'org-admin' })
    expect(screen.getByRole('button', { name: 'Tier 1' })).toBeTruthy()
    // ...but only the built-in one carries the badge.
  ...
  it('lets a custom role be edited (save) and deleted', async () => {
    renderPanel()
    await screen.findByRole('button', { name: 'Tier 1' })
