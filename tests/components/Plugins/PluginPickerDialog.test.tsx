...
describe('PluginPickerDialog', () => {
  ...
  it('has no force re-run toggle — manual UI runs always force', async () => {
    renderDialog()
    await screen.findByText('VirusTotal')
    expect(
      screen.queryByRole('checkbox', { name: /force re-run/i }),
  ...
  it('select-all ticks every plugin, and passes them all to onRun', async () => {
    ...
    // A manual UI run always forces — there is no toggle.
    expect(selection.force).toBe(true)
  })

  it('passes the chosen plugin ids on Run, always forcing', async () => {
    const { onRun } = renderDialog()
    await screen.findByText('VirusTotal')
