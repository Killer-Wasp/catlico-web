...
vi.mock('#/components/Observables/observablesQueries', () => ({
  ...
  observableRelatedCasesQueryOptions: () => ({
    queryKey: ['related-cases'],
...
describe('ObservableDetailDrawer — Run analyzers', () => {
  it('opens the picker and dispatches one run per selected plugin for this observable', async () => {
    ...
    // A manual UI run always forces (the picker bypasses the fresh-result dedup).
    ...
    expect(queueMock).toHaveBeenCalledWith('obs-42', { plugin_id: 'p2', force: true })

    await waitFor(() => expect(notifications.show).toHaveBeenCalledTimes(1))
  })

  it('forces each dispatch for a single selected plugin', async () => {
    renderDrawer()
